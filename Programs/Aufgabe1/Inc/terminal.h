 ...
 *
 * @date    27. Juli 2020
 * @brief   Dieses Modul implementiert ein einfaches Terminal f�r das LCD
...
typedef struct {
	...
	// Position f�r das naechste zu druckende Zeichen
	uint8_t curRow;
	uint8_t curCol;
	bool writeInACircle; // true : Schreibe weiter in Zeile 0, wenn die letzte Zeite voll ist
	                     // false : L�sche das Terminal, wenn alle Zeilen voll geschrieben sind.
} Terminal;
...
/**
  * @brief  Diese Funktion gibt eine Zeichen auf einem Terminal aus. 
	*         Zeilenumbrueche (\n) werden verarbeitet.
  * @param  c  Zeichen, der ausgegeben wird. 
  * @param  term  Das Terminal, auf dem der Text ausgegeben wird.
  * @retval none
  */
extern void printChar(char c, Terminal *term);
